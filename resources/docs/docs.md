...
The `home-routes` handler in the `avantqui.routes.home` namespace
...
The `home-page` function will in turn call the `avantqui.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `avantqui.handler` namespace:
...
Request middleware functions are located under the `avantqui.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `avantqui.dev-middleware` namespace found in
